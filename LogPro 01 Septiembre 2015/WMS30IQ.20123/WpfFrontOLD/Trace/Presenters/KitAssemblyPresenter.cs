...
namespace WpfFront.Presenters
{
    ...
    public interface IKitAssemblyPresenter
    {
        IKitAssemblyView View { get; set; }
    ...
    public class KitAssemblyPresenter : IKitAssemblyPresenter
    {
        ...
        public KitAssemblyPresenter(IUnityContainer container, IKitAssemblyView view, IShellPresenter region)
        {
            try
            {
                View = view;
                ...
                this.region = region;
                View.Model = this.container.Resolve<KitAssemblyModel>();
        ...
        public IKitAssemblyView View { get; set; }
        ...
        private void LoadDocuments()
        {
            try
            {
                ProcessWindow("Loading Documents ...", false);
        ...
        {
            ProcessWindow("Loading Late documents ...", false);
        ...
        private void OnLoadDetails(object sender, DataEventArgs<Document> e)
        {
            ...
            try
            {
                ProcessWindow("Loading Document " + e.Value.DocNumber + " ...", false);
                LoadDetails(e.Value);
                pw.Close();
        ...
        private void OnPickComponent(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Util.ShowError("Unit not Selected.");
                    return;
                }
                ...
                {
                    Util.ShowError("Error validating data. Please check data and try again.");
                    return;
                }

                if (!(int.Parse(View.TxtRcvQty.Text) > 0))
                {
                    Util.ShowError("Quantity is not valid.");
                    return;
                }

                ProcessWindow("Picking Component ...", false);
        ...
        private void ProcessPickingComponent(Label sourceLocation, int quantiy)
        {
            ...
            View.ProcessResult.Text = "Component Picked.";
        ...
        private void OnReceivingLabel(object sender, DataEventArgs<string> e)
        {
            ...
            {
                pw.Close();
                Util.ShowError("Label " + label.LabelCode + " does not exists.");
                return;
            }
        ...
        private void OnReceivingLabelList(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ProcessWindow("Picking Labels ...", false);
                ...
                //Refresh el Balancee
                RefreshBalance(View.Model.Document);
                View.ProcessResult.Text = "Label(s) Picked.";
        ...
        private void OnPickAtOnce(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ProcessWindow("Picking Components ...", false);

                View.ProcessResult.Text = "";
                service.PickAtOnce(View.Model.Document, sourceLocation, View.Model.Node);
                RefreshBalance(View.Model.Document);

                View.ProcessResult.Text = "Document " + View.Model.Document.DocNumber + " Picked at Once. Please review the balace for pending quantities.";
                Util.ShowMessage(View.ProcessResult.Text);
            ...
            {
                pw.Close();
                Util.ShowError("Document could not be picked at once. \n" + ex.Message);
            }
        }
...
/*
        ...
        */
        ...
        private void OnShowReceivingTicket(object sender, EventArgs e)
        {
            try
            {
                ProcessWindow("Generating Document ... ", false);
        ...
        private void OnConfirmOrder(object sender, EventArgs e)
        {
            ...
            {
                Util.ShowMessage("Error confirming document.");
                pw.Close();
            }
        }
        ...
        private void PrintLabels()
        {
            ...
            catch (Exception ex) { pw.Close();  Util.ShowError("Error printing. " + ex.Message); }
