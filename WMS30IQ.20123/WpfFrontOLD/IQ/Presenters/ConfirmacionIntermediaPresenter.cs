...
namespace WpfFront.Presenters
{
    ...
    public interface IConfirmacionIntermediaPresenter
    {
        IConfirmacionIntermediaView View { get; set; }
        ToolWindow Window { get; set; }
    }


    public class ConfirmacionIntermediaPresenter : IConfirmacionIntermediaPresenter
    {
        public IConfirmacionIntermediaView View { get; set; }
        ...
        private void OnAddLine(object sender, EventArgs e)
        {
            ...
            DataTable Validado = service.DirectSQLQuery(ConsultaValidar, "", "dbo.EquiposCLARO", Local);

            if (Validado.Rows.Count == 0)
            {
            ...
            try
            {
                ...
                if (Resultado.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        dr["codigo_sap"] = RegistroValidado.Rows[0]["codigo_sap"].ToString();
        ...
        private void OnConfirmarMovimiento(object sender, EventArgs e)
        {
            ...
            foreach (DataRowView item in View.ListadoEquiposAProcesar.SelectedItems)
            {
                //Creo la consulta para cambiar la ubicacion de la estiba
                ConsultaSQL += " UPDATE dbo.EquiposCLARO SET Ubicacion = '" + NuevaUbicacion + "', Estado = 'PARA DIAGNOSTICO', UA = '" + ((ComboBoxItem)View.UnidadAlmacenamiento.SelectedItem).Content.ToString() + "', CodigoEmpaque = '" + View.CodigoEmpaque.Text.ToString() + "' WHERE RowID = '" + item.Row["RowID"] + "'";

                ConsultaSQL += "exec sp_InsertarNuevo_Movimiento 'CONFIRMACION','CONFIRMACION','DIAGNOSTICO','" + View.CodigoEmpaque.Text.ToString() + "','" + item.Row["RowID"] + "','CONFIRMACION','UBICACIONPRODUCCION','" + this.user + "','';";
